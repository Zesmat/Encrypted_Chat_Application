"""
Hybrid System & Encoding Architect Module - Member 3
...
"""
import os
from .encoding import manual_hex_encode, manual_hex_decode
from . import aes
from . import rsa
...
def hybrid_encrypt(message_string, recipient_pub_key, on_debug=None):
    ...
    # 3. Encode to safe Hexadecimal strings for network transfer
    # (As architected in the Encoding module)
    encoded_ciphertext = manual_hex_encode(aes_ciphertext_bytes)
    _log("📝 [HEX] Encoded Ciphertext for Network",
    ...
    envelope = {
        ...
        "aes_ciphertext_hex": encoded_ciphertext    # String
    }
...
def hybrid_decrypt(envelope, recipient_priv_key, on_debug=None):
    ...
    encoded_ciphertext = envelope["aes_ciphertext_hex"]
    ...
    plaintext_string = aes.aes_decrypt(encoded_ciphertext, session_key_hex)
    _log("✅ [AES] Decrypted Plaintext", plaintext_string)
